...
fn main() {
    ...
    // calculate answer
    let min_i = elves.iter().map(|elf| elf[0]).min().unwrap();
    let max_i = elves.iter().map(|elf| elf[0]).max().unwrap() + 1;
    let min_j = elves.iter().map(|elf| elf[1]).min().unwrap();
    let max_j = elves.iter().map(|elf| elf[1]).max().unwrap() + 1;
    let area = ((max_i - min_i) * (max_j - min_j)) as usize;
    println!("{}", area - elves.len())
}
